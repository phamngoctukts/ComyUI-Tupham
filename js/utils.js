...
export function CUSTOM_PROMPT(node, inputName, val, func, config = {}) {
    return {
        widget: node.addWidget(
            "STRING", // Loại widget là "text"
            inputName, // Tên hiển thị của widget
            val, // Giá trị mặc định
            func, // Hàm callback khi giá trị thay đổi
            Object.assign({}, {multiline: true, placeholder: "gõ prpmpt..."}, config) // Cấu hình widget
        ),
    };
}
...
export function transformFunc_text(widget, value, node, index) {
	node.properties["values"][node.widgets[node.index].value][index] = widget.value
	if (node.widgets_values) { 
		node.widgets_values[3] = node.properties["values"]
	}
}
